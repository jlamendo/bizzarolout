...
internals.factory.prototype.fuzz = function (callback) {
    ...
    Async.mapLimit(table, 3, function (route, next) {
        ...
        self.log(['bizzarolout', 'info'], 'Fuzzing Route: ' + route.path + ' AUTH: ' + route.auth + ' Params: ' + JSON.stringify(params));
        ...
        var generateFuzz = function () {
            ...
            params.query.forEach(function (param) {
                ...
                if (fuzzed.url.indexOf('{' + param.name + '}') !== -1) {
                    fuzzed.url = fuzzed.url.replace('{' + param.name + '}', self.fuzzParam(param));
                }
                else {
        ...
        var results = [];
        for (var i = 0, l = self.maxIterations; i < l; ++i) {
            results.push({});
        }
        ...
        Async.mapSeries(results, function (result, next) {
            ...
            results = results.filter(function (result) { return result.statusCode >= 400; });
            self.log(['bizzarolout', 'info'], 'finished route: ' + route.path + ' ' + results.length + ' errors');
            next(err, results);
        });
    }, function (err, results) {
